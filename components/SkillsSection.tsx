...
const SKILL_ICONS: Record<string, IconDef> = {
  ...
  "GitHub":       { icon: SiGithub,      color: "#334155" },
  ...
  return (
    <section id="skills" className="relative py-20 md:py-32 overflow-hidden">
      ...
      <div className="pointer-events-none absolute inset-0 -z-10">
        <div className="absolute left-1/2 top-1/2 h-[500px] w-[900px] -translate-x-1/2 -translate-y-1/2 rounded-full bg-red-50/60 blur-[120px]" />
        <div className="absolute left-1/4 top-1/3 h-[200px] w-[200px] rounded-full bg-rose-50/50 blur-[80px]" />
        <div className="absolute right-1/4 bottom-1/3 h-[200px] w-[200px] rounded-full bg-red-50/40 blur-[80px]" />
      </div>
...
function CategoryDivider({ label }: { label: string }) {
  return (
    <div className="mx-5 flex shrink-0 items-center gap-2">
      <span className="h-px w-6 bg-gradient-to-r from-transparent to-red-200" />
      <span className="h-1.5 w-1.5 shrink-0 rotate-45 bg-red-300" />
      <span className="select-none text-[9px] font-black uppercase tracking-[0.28em] text-red-400/70">
        {label}
      </span>
      <span className="h-1.5 w-1.5 shrink-0 rotate-45 bg-red-300" />
      <span className="h-px w-6 bg-gradient-to-l from-transparent to-red-200" />
    </div>
  );
}
...
function SkillChip({ skill, size = "md" }: { skill: SkillItem; size?: "md" | "lg" }) {
  ...
  return (
    <div
      className={`group relative flex shrink-0 cursor-default items-center overflow-hidden rounded-xl border border-slate-100 bg-white/80 backdrop-blur-sm transition-all duration-300 hover:border-red-200 hover:bg-white hover:shadow-lg hover:shadow-red-50 ${
    ...
    >
      ...
      <span
        ...
        style={{
          background: `radial-gradient(ellipse at 20% 50%, ${skill.color}12 0%, transparent 65%)`,
        }}
      />
      ...
      <span
        ...
        style={{ background: `linear-gradient(90deg, transparent, ${skill.color}40, transparent)` }}
      />
      <Icon
        className={`relative z-10 shrink-0 text-slate-400 transition-all duration-300 group-hover:scale-110 group-hover:text-(--brand) group-hover:drop-shadow-[0_0_6px_var(--brand)] ${
          isLg ? "h-[20px] w-[20px]" : "h-[17px] w-[17px]"
        }`}
        aria-hidden="true"
      />
      <span
        className={`relative z-10 whitespace-nowrap font-medium tracking-wide text-slate-600 transition-colors duration-300 group-hover:text-slate-800 ${
