      ...
      <main className="relative z-10 px-5 pt-5 pb-28 sm:px-8 md:pt-10">
        <div className="mx-auto max-w-5xl">
              ...
              <Reveal delay={80} className="md:col-span-3">
                <div className="space-y-5">
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                    {about.birthdate && (
                      <span className="inline-flex items-center gap-1.5 rounded-lg border border-red-100/80 bg-white/60 px-3 py-1.5 text-xs text-slate-600 backdrop-blur-sm">
                        <span className="h-1.5 w-1.5 rounded-full bg-red-500" />
                        <AgeDisplay birthdate={about.birthdate} className="font-semibold text-slate-900" />
                      </span>
                    )}
                    {about.meta.map(({ icon, text }) => (
                      <span
                        key={text}
                        className="flex items-center gap-2 rounded-lg border border-red-100/80 bg-white/60 px-3 py-1.5 text-xs text-slate-600 backdrop-blur-sm"
                      >
                        <FontAwesomeIcon
                          icon={icon}
                  ...
                  <div className="space-y-3 text-sm leading-relaxed text-slate-600">
                    ...
                    <p>
                      Selama lebih dari{" "}
                      <span className="font-semibold text-slate-900">2 tahun</span> berkecimpung di dunia{" "}
                      <span className="font-semibold text-slate-900">freelance</span>, saya telah
                  ...
                  {about.highlights.length > 0 && (
                    <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
                      {about.highlights.map((h) => (
                        <div
                          key={h.label}
                          className="rounded-2xl border border-red-100/60 bg-white/70 px-4 py-3.5 text-center backdrop-blur-sm transition-all duration-200 hover:border-red-200 hover:shadow-sm"
                        >
                          <p className="text-xl font-black gradient-text">{h.value}</p>
                          <p className="mt-0.5 text-[11px] text-slate-500">{h.label}</p>
                        </div>
                      ))}
                    </div>
                  )}
                ...
                <div className="relative mx-auto max-w-55 sm:max-w-xs lg:max-w-57.5">
                  ...
                  <div className="relative aspect-3/4 overflow-hidden rounded-2xl border border-red-100/60 bg-white shadow-xl shadow-red-100/20">
                    ...
                    <div className="absolute inset-x-0 bottom-0 p-4">
                      ...
                      <p className="mt-0.5 text-[11px] font-medium text-red-600">Full Stack Dev &amp; SEO Specialist</p>
            ...
            <section className="mb-16">
              <Reveal>
                <div className="mb-8 flex items-center gap-3">
                  <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-gradient-to-br from-red-50 to-rose-50 ring-1 ring-red-100">
                    <FontAwesomeIcon icon={faGraduationCap} className="h-4 w-4 text-red-500" />
                  </div>
                  <div>
                    <p className="text-[10px] font-bold tracking-widest uppercase gradient-text">
                      Education
                    </p>
                    <h2 className="text-lg font-black text-slate-900 sm:text-xl">Riwayat Pendidikan</h2>
                  </div>
                </div>
              </Reveal>
              <Reveal delay={60}>
                <div className="rounded-2xl border border-red-100/60 bg-white/70 p-5 backdrop-blur-sm sm:p-6">
                  {about.education.map((edu, i) => (
                    <div key={edu.school} className="relative flex gap-4 sm:gap-5">
                      {/* Connector line */}
                      <div className="flex flex-col items-center">
                        <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full border border-red-200/80 bg-gradient-to-br from-red-50 to-rose-50">
                      ...
                      <div className={`min-w-0 flex-1 ${i < about.education.length - 1 ? "pb-7" : ""}`}>
                        <div className="flex flex-wrap items-start justify-between gap-2">
                          <div>
                            ...
                            <p className="mt-0.5 text-xs text-slate-500">{edu.school}</p>
                          </div>
                          <div className="flex shrink-0 flex-col items-end gap-1.5">
                            ...
                            {edu.note && (
                              <span className="flex items-center gap-1 text-[10px] text-slate-500">
                                <FontAwesomeIcon icon={faStar} className="h-2.5 w-2.5 text-amber-500/70" />
                                {edu.note}
                              </span>
                            )}
                          </div>
              ...
              <Reveal>
                <div className="mb-8 flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <h2 className="text-lg font-black text-slate-900 sm:text-xl">Pengalaman Kerja</h2>
                  </div>
                </div>
              </Reveal>
              <Reveal delay={60}>
              ...
              <Reveal>
                <div className="mb-8 flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <h2 className="text-lg font-black text-slate-900 sm:text-xl">Riwayat Organisasi</h2>
                  </div>
                </div>
              </Reveal>
              <Reveal delay={60}>
              ...
              <Reveal>
                <div className="mb-8 flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <h2 className="text-lg font-black text-slate-900 sm:text-xl">Sertifikat</h2>
                  </div>
                </div>
              </Reveal>
          ...
          {!hasEducation && !hasExperience && !hasOrganizations && !hasCertificates && (
            <Reveal>
              <div className="rounded-2xl border border-red-100/60 bg-white/70 px-6 py-16 text-center backdrop-blur-sm">
